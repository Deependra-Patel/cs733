...
func TestAppend(t *testing.T) {
	...
	expect2(t, errorMessage, response, Commit{index: -1, data: data, err: "ERR_NOT_LEADER"})
	...
	sm.clientCh <- AppendEv{data: data}
	response = <-sm.actionCh
	expect2(t, errorMessage, response, LogStore{index: 4, data: data, term: sm.term})
	for _, peer := range initialSm.peers {
		response = <-sm.actionCh
		expect2(t, errorMessage, response, Send{peer,
...
func TestTimeout(t *testing.T) {
	...
	expect2(t, errorMessage, sm.state, "Candidate")
	expect2(t, errorMessage, sm.voteCount, 1)
	expect2(t, errorMessage, sm.votedFor, initialSm.id)
	expect2(t, errorMessage, sm.term, initialSm.term+1)
	response := <-sm.actionCh
	expect2(t, errorMessage, response, StateStore{currentTerm: initialSm.term + 1, votedFor: initialSm.id})
	response = <-sm.actionCh
	expect2(t, errorMessage, response, Alarm{t: sm.ElectionTimeout})
	for _, peer := range sm.peers {
		response = <-sm.actionCh
		expect2(t, errorMessage, response, Send{peer, VoteReqEv{
	...
	initialSm = getSampleSM("Leader")
	sm.timeoutCh <- TimeoutEv{}
	expect2(t, errorMessage, sm.state, "Leader")
	expect2(t, errorMessage, sm.term, initialSm.term)
	response = <-sm.actionCh
	expect2(t, errorMessage, response, Alarm{t: sm.HeartbeatTimeout})
	for _, peer := range sm.peers {
		response = <-sm.actionCh
		if len(initialSm.log) != initialSm.nextIndex[peer] {
			expect2(t, errorMessage, response, Send{peer, AppendEntriesReqEv{
...
func TestAppendEntriesReq(t *testing.T) {
	...
	expect2(t, errorMessage, sm.state, "Follower")
...
func TestAppendEntriesResp(t *testing.T) {
	...
	expectActions(t, errorMessage, sm, expectedActions)
	<-sm.actionCh //For dummy Finish
	expect2(t, errorMessage, sm.commitIndex, 2)
	checkEmptyChannel(t, errorMessage, sm)
}
...
func TestVoteResp(t *testing.T) {
	...
	expect2(t, errorMessage, "Candidate", sm.state)
	<-sm.actionCh //For dummy Finish
	...
	sm.netCh <- VoteRespEv{Term: sm.term, VoteGranted: true}
	expect2(t, errorMessage, "Leader", sm.state)
	response := <-sm.actionCh
	expect2(t, errorMessage, response, Alarm{t: sm.HeartbeatTimeout})
	for _, peer := range sm.peers {
		response = <-sm.actionCh
		expect2(t, errorMessage, sm.nextIndex[peer], len(sm.log))
		expect2(t, errorMessage, sm.matchIndex[peer], 0)
		expect2(t, errorMessage, response, Send{peer, AppendEntriesReqEv{
			Term: initialSm.term, LeaderId: initialSm.id, PrevLogIndex: len(initialSm.log) - 1,
			PrevLogTerm: initialSm.log[len(initialSm.log)-1].Term, Entries: nil,
			LeaderCommit: initialSm.commitIndex}})
...
func expectActions(t *testing.T, message string, sm *StateMachine, expectedActions []interface{}) {
	for _, expectedAction := range expectedActions {
		response := <-sm.actionCh
		expect2(t, message, response, expectedAction)
	}
}

func expect2(t *testing.T, message string, response interface{}, expected interface{}) {
